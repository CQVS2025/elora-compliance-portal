...
export default function NotificationSettings() {
  ...
  const loadPreferences = async () => {
    try {
      ...
      if (prefs.length > 0) {
        ...
        const created = await base44.entities.NotificationPreferences.create(defaultPrefs);
  ...
  const saveMutation = useMutation({
    mutationFn: async (updatedPrefs) => {
      await base44.entities.NotificationPreferences.update(preferences.id, updatedPrefs);
    },
    onSuccess: () => {
