...
export default function Refills2({ selectedCustomer, selectedSite, dateRange }) {
  ...
  const filteredRefills = refills.filter(refill => 
    refill.siteName?.toLowerCase().includes(searchQuery.toLowerCase()) ||
    refill.customerName?.toLowerCase().includes(searchQuery.toLowerCase())
  );
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-slate-100 border-b border-slate-200">
                    ...
                    <th className="px-4 py-3 text-right text-xs font-semibold text-slate-700 uppercase">Last Tank Level (m)</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    >
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-2 text-sm">
                          ...
                          <span className="text-slate-700">
                            {moment(refill.refillDate || refill.date).format('MMM D, YYYY')}
                          </span>
                        </div>
                      </td>
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-slate-800 font-medium">{refill.siteName}</span>
                        </div>
                      </td>
                      ...
                      <td className="px-4 py-4 text-right">
                        <span className="text-slate-800 font-semibold">
                          {refill.quantity ? refill.quantity.toLocaleString() : 'N/A'}
                        </span>
                      </td>
                      <td className="px-4 py-4 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <span className={`text-lg font-bold ${
                            refill.lastTankLevel !== undefined && refill.lastTankLevel !== null
                              ? refill.lastTankLevel < 0.5 
                                ? 'text-red-600' 
                                : refill.lastTankLevel < 1.0
                                  ? 'text-orange-500'
                                  : 'text-[#7CB342]'
                              : 'text-slate-400'
                          }`}>
                            {refill.lastTankLevel !== undefined && refill.lastTankLevel !== null
                              ? `${refill.lastTankLevel.toFixed(2)}m`
                              : 'N/A'}
                          </span>
                        </div>
                      </td>
